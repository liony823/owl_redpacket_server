 ...
 * @date 2024-08-10 16:56
 */
@Slf4j
@Service("luck_RedPacketExpireServiceImpl")
public class LuckRedPacketExpireServiceImpl implements RedPacketExpireService {
    @Autowired
    ...
    public void handlerRedPacketExpire(RedPacket redPacket) {
        ...
        Query recordQuery = new Query(
                Criteria.where("red_packet_id").is(redPacketId)
                        .and("record_type").is(RedPacketRecordType.RECEIVE_FROM_GROUP.getCode())
        );

        //查询红包记录表 查询已领取条数
        List<RedPacketRecord> redPacketRecords = mongoTemplate.find(recordQuery, RedPacketRecord.class);

        //如果已领取条数等于总条数 说明已领取完毕
        if (redPacketRecords.size() == redPacket.getTotalCount()) {
            return;
        }
        //如果未领取完毕 退还剩余红包 将已录取的金额累加 使用总金额减去已领取金额 退还剩余金额
        double receiveMoney = redPacketRecords.stream().mapToDouble(RedPacketRecord::getAmount).sum();
        double expireMoney = redPacket.getTotalAmount() - receiveMoney;
        ...
        update.inc("balance", expireMoney);
        ...
        //发送退款消息
        sendRedPacketRefundMsg(redPacket.getSenderId(), redPacket.getGroupID(), redPacketId);
    }
    ...
    public void sendRedPacketRefundMsg(String sendId, String groupId, String redPacketId) {
...
//        content.put("content", "红包过期退款");
//        content.put("customType", 1008);
