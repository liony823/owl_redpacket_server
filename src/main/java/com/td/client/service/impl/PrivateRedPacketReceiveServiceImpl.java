 ...
 */
@Service("private_RedPacketReceiveServiceImpl")
public class PrivateRedPacketReceiveServiceImpl implements RedPacketReceiveService {
    ...
    public ReceiveRedPacketResponseDto receiveRedPacket(ReceiveRedPacketRequestDto receiveRedPacketRequestDto) {
        ...
        Object redPacketCatch = redisUtils.getCacheObject(RedisKeyEnums.RED_PACKET_PRIVATE.getKey() + receiveRedPacketRequestDto.getRedPacketId());
        if (redPacketCatch == null) {
            throw new CustomException("红包已领完");
        }

//        if (redPacket.getSenderId().equals(currUserid)) {
//            throw new CustomException("不能领取自己的红包");
//        }
        ...
        Query recordQuery = new Query(
                Criteria.where("red_packet_id").is(receiveRedPacketRequestDto.getRedPacketId())
                        ...
                        .and("record_type").is(RedPacketRecordType.RECEIVE_FROM_USER.getCode())
        );
        ...
        RedPacketRecord record = new RedPacketRecord()
                ...
                .setRecordType(RedPacketRecordType.RECEIVE_FROM_USER.getCode())
        ...
        //删除缓存
        redisUtils.deleteObject(RedisKeyEnums.RED_PACKET_PRIVATE.getKey() + receiveRedPacketRequestDto.getRedPacketId());
